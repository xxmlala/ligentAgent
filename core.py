...
def train_valueNet(cfg, seed: int, log_dict: dict, idx: int, logger: logging.Logger, barrier: Optional[mp.Barrier]):
    ...
    agent = instantiate(cfg.agent, preprocess_net=feature_net, device=device, policy_freeze=True)

    root_dir = hydra.utils.get_original_cwd()
    agent.load_full_path(os.path.join(root_dir,"models/step_69632_model_seed_3407_"))
    ...
    for step in range(cfg.vec_envs, cfg.timesteps + 1, cfg.vec_envs):
        ...
        if cfg.vec_envs > 1 and done.any():
            ...
            other_utils.write_to_dict(local_log_dict, 'train_returns', round(cumulate_reward,3), using_mp)
...
def train(cfg, seed: int, log_dict: dict, idx: int, logger: logging.Logger, barrier: Optional[mp.Barrier]):
    ...
    agent.load_full_path("/home/liuan/workspace/drl_project/ligentAgent/models/step_69632_model_seed_3407_")               
    cfg = DotMap(OmegaConf.to_container(cfg.train, resolve=True))
    ...
    for step in range(cfg.vec_envs, cfg.timesteps + 1, cfg.vec_envs):
        ...
        if cfg.vec_envs > 1 and done.any():
            ...
            state = env.reset()
            env_decoder.reset()
            # logger.info(f"It gets {round(cumulate_reward,3)} sum reward, costs {elspsed_step} steps, distance_info: {distance_info}")
            last_reset_time = time.time()
            done, blocked = False, False
            # other_utils.write_to_dict(local_log_dict, 'train_returns', info['episode']['r'].item(), using_mp)
            other_utils.write_to_dict(local_log_dict, 'train_returns', round(cumulate_reward,3), using_mp)
